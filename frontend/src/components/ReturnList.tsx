...
export const ReturnList: React.FC<ReturnListProps> = ({ returns, onViewDetails, onUpdateStatus }) => {
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <tbody className="text-gray-700">
          {returns.length === 0 ? (
            <tr>
              <td colSpan={7} className="text-center py-4">No returns found.</td>
            </tr>
          ) : (
            returns.map((returnItem) => (
              <tr key={returnItem.id} className="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td className="py-3 px-4">{new Date(returnItem.return_date).toLocaleDateString()}</td>
                ...
                <td className="py-3 px-4">
                  ...
                  <select
                    ...
                    onChange={(e) => onUpdateStatus(returnItem.id, e.target.value as 'pending' | 'approved' | 'rejected' | 'completed')}
