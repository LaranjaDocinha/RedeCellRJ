  ...
  return (
    <Box component="form" onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} sx={{ mt: 1 }}>
      <Grid container spacing={2}>
        <Grid size={{ xs: 12 }}>
          <TextField fullWidth label="Nome da Regra" name="name" size="small" value={formData.name} onChange={handleChange} required />
        </Grid>
        <Grid size={{ xs: 6 }}>
        ...
        <Grid size={{ xs: 12 }}>
          <FormControlLabel control={<Switch name="is_active" checked={formData.is_active} onChange={handleChange} />} label={<Typography variant="body2" sx={{ fontWeight: 400 }}>Regra Ativa</Typography>} />
        </Grid>
      </Grid>
