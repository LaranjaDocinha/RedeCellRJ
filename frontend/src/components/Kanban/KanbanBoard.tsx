...
import { getBoard, moveCard, createCard, deleteCard, moveColumn, updateCard } from '../../hooks/useKanbanApi';
...
const KanbanBoard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBoardAndUsers = async () => {
      try {
        ...
        const usersResponse = await fetch('/users'); // Assuming /users endpoint exists and returns { id, name }
        if (!usersResponse.ok) throw new Error(`HTTP error! status: ${usersResponse.status}`);
        const usersData = await usersResponse.json();
        setAvailableAssignees(usersData.map((user: any) => ({ id: user.id, name: user.name || user.email })));
  ...
  const handleDragEnd = (event: any) => {
    ...
    setBoard(prevBoard => {
      ...
      const newColumn = prevBoard.find(col => col.id === over.id || col.cards.some(card => card.id === over.id));
    ...
    setBoard(prevBoard => {
      return prevBoard.map(col => {
        if (col.id === updatedCardData.column_id) {
          return { ...col, cards: col.cards.map(card => card.id === updatedCardData.id ? updatedCardData : card) };
        }
        return col;
      });
    });
  ...
  if (loading) return <p>Carregando quadro...</p>;
  if (error) return <p>{error}</p>;
  if (!board.length) return <p>Nenhum dado encontrado.</p>;
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Kanban Card">
        {editingCard && (
          <KanbanCardForm
