      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={8}>
            <Typography variant="h6" sx={{ mb: 3, fontWeight: 400 }}>Próximas Visitas</Typography>
            <Stack spacing={2.5}>
                        ...
                        <Grid container alignItems="center">
                          <Grid item xs={12} sm={2}>
                            <Box sx={{ textAlign: 'center', borderRight: { sm: '1px solid' }, borderColor: 'divider', pr: { sm: 2 } }}>
                              <Typography variant="h4" fontWeight={400}>{moment(app.appointment_date).format('DD')}</Typography>
                              <Typography variant="overline" fontWeight={400} color="text.secondary">{moment(app.appointment_date).format('MMM')}</Typography>
                            </Box>
                          </Grid>
                          <Grid item xs={12} sm={7} sx={{ pl: { sm: 3 }, mt: { xs: 2, sm: 0 } }}>
                            <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                              <ToolIcon sx={{ fontSize: 18, color: 'primary.main' }} />
                              <Typography variant="subtitle1" fontWeight={400}>{app.service_type}</Typography>
                            </Box>
                            <Box display="flex" alignItems="center" gap={2}>
                              ...
                              <Chip 
                                ...
                                sx={{ height: 18, fontSize: '0.6rem', fontWeight: 400, borderRadius: '6px' }} 
                              />
                            </Box>
                          </Grid>
                          <Grid item xs={12} sm={3} sx={{ textAlign: 'right', mt: { xs: 2, sm: 0 } }}>
                            <Button size="small" color="error" startIcon={<CancelIcon />} sx={{ fontWeight: 400 }}>Cancelar</Button>
                          </Grid>
                        </Grid>
                      </CardContent>
                    </Card>
          ...
          <Grid item xs={12} md={4}>
            <Paper sx={{ p: 3, borderRadius: '24px', bgcolor: 'action.hover', border: '1px solid', borderColor: 'divider' }}>
              <Typography variant="h6" fontWeight={400} gutterBottom>Status da Oficina</Typography>
              <Box mt={3}>
                <Box display="flex" justifyContent="space-between" mb={1}>
                  <Typography variant="body2" fontWeight={400}>Ocupação Hoje</Typography>
                  <Typography variant="body2" fontWeight={400} color="success.main">Normal</Typography>
                </Box>
              ...
              <Typography variant="subtitle2" fontWeight={400} gutterBottom>Agendamento Rápido</Typography>
              ...
              <Button fullWidth variant="outlined" sx={{ mt: 3, borderRadius: '12px', fontWeight: 400, textTransform: 'none' }} startIcon={<HistoryIcon />}>
        ...
        <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)}>
          <Box sx={{ 
            ...
            <Typography variant="h5" sx={{ fontWeight: 400, mb: 1 }}>Agendar Horário</Typography>
                ...
                {availableServiceTypes.map((s) => (
                  <MenuItem key={s.label} value={s.label}>
                    <Box display="flex" alignItems="center" gap={1.5}>
                      ...
                      <Typography variant="body2" fontWeight={400}>{s.label}</Typography>
                    </Box>
                  </MenuItem>
                ))}
              </Select>
            ...
            <Box display="flex" gap={2}>
              ...
              <Button fullWidth variant="contained" onClick={handleCreateAppointment} sx={{ borderRadius: '12px', fontWeight: 400 }}>Confirmar Agendamento</Button>
            </Box>
          </Box>
        </Modal>
      </Container>
