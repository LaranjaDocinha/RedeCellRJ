...
const ProductListItem = React.memo(({ product, onEdit, onDelete, onQuickView }) => {
  ...
  const getPrice = () => {
    ...
    return <NumericFormat value={price} displayType={'text'} thousandSeparator="." decimalSeparator="," prefix={'R$ '} />;
  };
    ...
    <div className={`product-list-item-wrapper ${isSelected ? 'selected' : ''}`}>
      <div className="product-list-item">
        ...
        <div className="product-list-item__info" title={name}>
          ...
          <small className="text-muted" title={category?.name}>{category?.name || 'Sem categoria'}</small>
        </div>
        ...
        <div className="product-list-item__type">
          <Badge color={productType === 'physical' ? 'info' : 'secondary'} pill>
            {productType === 'physical' ? 'Físico' : 'Serviço'}
          </Badge>
        </div>
        <div className="product-list-item__actions">
          <Button size="sm" className="themed-action-btn" onClick={() => onQuickView(product)}>
            <i className="bx bx-search-alt"></i>
          </Button>
          <Button size="sm" className="themed-action-btn" onClick={() => onEdit(product)}>
            <i className="bx bx-pencil"></i>
          </Button>
          <Button size="sm" className="themed-action-btn" onClick={() => onDelete(product)}>
