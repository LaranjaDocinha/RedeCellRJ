...
class LoyaltyTierService {
  async createTier(name: string, min_points: number, description: string, benefits: any) {
  ...
  async updateTier(
    ...
    const result = await pool.query(
      ...
      [name, min_points, description, benefits, id],
    );
    return result.rows[0];
  }
  ...
  async updateCustomerTier(customerId: number) {
    const customerRes = await pool.query(
      'SELECT u.loyalty_points FROM users u JOIN customers c ON u.id = c.user_id WHERE c.id = $1',
      [customerId],
    );
    ...
    const tiersRes = await pool.query(
      'SELECT id, min_points FROM loyalty_tiers ORDER BY min_points DESC',
    );
