...
export const updateProductCorrelations = async (productIds: number[]) => {
...
export const getProductRecommendations = async (productId: number, customerId?: number) => {
  let query = `
  ...
  if (customerId) {
    ...
    const customerHistoryRes = await pool.query(
      'SELECT DISTINCT si.product_id FROM sales s JOIN sale_items si ON s.id = si.sale_id WHERE s.customer_id = $1',
      [customerId],
    );
...
export const getPersonalizedRecommendations = async (customerId: number) => {
  const query = `
...
export const getCartUpsellRecommendations = async (cartProductIds: number[]) => {
  if (cartProductIds.length === 0) return [];

  const query = `
    SELECT p.*, SUM(pc.purchase_count) as score
    FROM product_correlations pc
    JOIN products p ON p.id = (CASE WHEN pc.product_a_id = ANY($1) THEN pc.product_b_id ELSE pc.product_a_id END)
    WHERE (pc.product_a_id = ANY($1) OR pc.product_b_id = ANY($1))
    AND p.id != ALL($1)
    GROUP BY p.id
    ORDER BY score DESC
    LIMIT 5
  ...
  const result = await pool.query(query, [cartProductIds]);
  return result.rows;
};


export const getCollaborativeRecommendations = async (customerHistoryProductIds: number[], excludeProductIds: number[] = []) => {
  if (customerHistoryProductIds.length === 0) return [];

  // Encontrar produtos frequentemente comprados por clientes com histórico similar,
  // excluindo produtos já comprados ou no carrinho.
  const query = `
    SELECT p.*, COUNT(DISTINCT s.customer_id) as common_customer_count
    FROM product_correlations pc
    JOIN products p ON p.id = (CASE WHEN pc.product_a_id = ANY($1) THEN pc.product_b_id ELSE pc.product_a_id END)
    WHERE (pc.product_a_id = ANY($1) OR pc.product_b_id = ANY($1))
    AND p.id != ALL($1) -- Excluir produtos do histórico/carrinho
    ${excludeProductIds.length > 0 ? `AND p.id != ALL($2)` : ''}
    GROUP BY p.id
    ORDER BY common_customer_count DESC
    LIMIT 5;
  `;

  const params: any[] = [customerHistoryProductIds];
