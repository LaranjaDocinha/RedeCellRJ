...
interface CreatePartSupplierPayload {
  part_id: number;
  supplier_id: number;
  cost: number;
  lead_time_days?: number;
...
interface UpdatePartSupplierPayload {
  cost?: number;
  lead_time_days?: number;
  supplier_part_number?: string;
}
...
class PartSupplierService {
  async getSuppliersForPart(partId: number): Promise<any[]> {
    const query = `
      SELECT
        s.id,
        s.name,
  ...
  async addSupplierToPart(payload: CreatePartSupplierPayload): Promise<PartSupplier> {
    const { part_id, supplier_id, cost, lead_time_days, supplier_part_number } = payload;

    // Check for duplicates
    const existing = await pool.query(
      'SELECT id FROM part_suppliers WHERE part_id = $1 AND supplier_id = $2',
      [part_id, supplier_id],
    );
  ...
  async updateSupplierForPart(
    partId: number,
    supplierId: number,
    payload: UpdatePartSupplierPayload,
  ...
  async removeSupplierFromPart(partId: number, supplierId: number): Promise<boolean> {
    const result = await pool.query(
      'DELETE FROM part_suppliers WHERE part_id = $1 AND supplier_id = $2 RETURNING id',
      [partId, supplierId],
    );
