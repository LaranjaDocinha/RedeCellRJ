...
interface CreatePaymentGatewayPayload {
  name: string;
  type: string;
  config: any;
  is_active?: boolean;
...
interface UpdatePaymentGatewayPayload {
  name?: string;
  type?: string;
  config?: any;
  is_active?: boolean;
...
export const paymentGatewayService = {
  ...
  async createGateway(payload: CreatePaymentGatewayPayload): Promise<PaymentGateway> {
    const { name, type, config, is_active = true } = payload;
    try {
      const result = await pool.query(
        'INSERT INTO payment_gateways (name, type, config, is_active) VALUES ($1, $2, $3, $4) RETURNING *',
  ...
  async updateGateway(id: number, payload: UpdatePaymentGatewayPayload): Promise<PaymentGateway> {
    ...
    if (fields.length === 0) {
      ...
      if (!existing) throw new NotFoundError('Payment gateway not found.');
  ...
  async processPayment(gatewayId: number, amount: number, paymentData: any): Promise<any> {
