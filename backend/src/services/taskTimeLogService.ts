...
export const startTimer = async (userId: string, serviceOrderId: number) => {
...
export const stopTimer = async (userId: string, serviceOrderId: number, notes?: string) => {
  const result = await getPool().query(
...
export const getLogsForServiceOrder = async (serviceOrderId: number) => {
  const result = await getPool().query(
...
export const getActiveTimerForUser = async (userId: string, serviceOrderId: number) => {
  const result = await getPool().query(
    'SELECT * FROM task_time_log WHERE user_id = $1 AND service_order_id = $2 AND end_time IS NULL',
    [userId, serviceOrderId],
  );
