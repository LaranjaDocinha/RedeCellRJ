...
export const twoFactorAuthService = {
  ...
  async verifyToken(userId: string, token: string): Promise<boolean> {
    const userResult = await pool.query('SELECT two_factor_secret FROM users WHERE id = $1', [
      userId,
    ]);
  ...
  async is2FAEnabled(userId: string): Promise<boolean> {
    const userResult = await pool.query('SELECT two_factor_enabled FROM users WHERE id = $1', [
      userId,
    ]);
    const user = userResult.rows[0];
