...
export const gamificationEngine = {
  async processSale(_userId: string, _totalAmount: number, _items: any[], _customerId?: string) {
    try {
  ...
  async awardXP(userId: string, xpAmount: number) {
    ...
    try {
      ...
      await pool.query('UPDATE users SET xp = $1, level = $2, updated_at = NOW() WHERE id = $3', [
        newXP,
        newLevel,
        userId,
      ]);
