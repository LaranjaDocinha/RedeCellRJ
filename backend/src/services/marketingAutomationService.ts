...
class MarketingAutomationService {
  async createAutomation(
    ...
    try {
      ...
      const automationResult = await client.query(
        'INSERT INTO automations (name, trigger_type, trigger_config) VALUES ($1, $2, $3) RETURNING *',
      ...
      for (const step of steps) {
        await client.query(
          'INSERT INTO automation_steps (automation_id, type, payload, order_index) VALUES ($1, $2, $3, $4)',
          [newAutomation.id, step.type, step.payload, step.order_index],
        );
      }
  ...
  async updateAutomation(
    ...
    try {
      ...
      const automationResult = await client.query(
        'UPDATE automations SET name = $1, is_active = $2, trigger_type = $3, trigger_config = $4, updated_at = current_timestamp WHERE id = $5 RETURNING *',
        [name, is_active, trigger_type, trigger_config, id],
      );
      ...
      for (const step of steps) {
        await client.query(
          'INSERT INTO automation_steps (automation_id, type, payload, order_index) VALUES ($1, $2, $3, $4)',
          [id, step.type, step.payload, step.order_index],
        );
      }
  ...
  async processPendingRuns() {
    const runs = await pool.query(
      "SELECT * FROM automation_runs WHERE status = 'active' AND next_step_due_at <= NOW()",
    );
  ...
  async handleEvent(eventName: string, eventPayload: any) {
    const automations = await pool.query(
      'SELECT * FROM automations WHERE trigger_type = $1 AND is_active = true',
      [eventName],
    );
    ...
    for (const automation of automations.rows) {
      ...
      if (automation.trigger_config?.product_ids) {
        const saleProductIds = new Set(eventPayload.items.map((item: any) => item.product_id));
        shouldTrigger = automation.trigger_config.product_ids.some((id: number) =>
          saleProductIds.has(id),
        );
      }

      if (shouldTrigger) {
        await pool.query(
          'INSERT INTO automation_runs (automation_id, customer_id, status, current_step, trigger_payload) VALUES ($1, $2, $3, $4, $5)',
          [automation.id, eventPayload.sale.customerId, 'active', 0, eventPayload],
        );
      }
    }
  }
