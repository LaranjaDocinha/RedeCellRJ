...
export const calculateClv = async (customerId: string) => {
  const pool = getPool();
  ...
  try {
    ...
    if (first_purchase_date && last_purchase_date) {
      const diffTime = Math.abs(
        new Date(last_purchase_date).getTime() - new Date(first_purchase_date).getTime(),
      );
