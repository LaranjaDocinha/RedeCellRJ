...
export const smartPricingService = {
  ...
  async applyRule(rule: any) {
    ...
    if (condition_type === 'low_turnover') {
      ...
      const res = await pool.query(
        'SELECT id, price, cost_price, sku FROM product_variations WHERE stock_quantity >= $1',
        [minStock],
      );
      productsToUpdate = res.rows;
    }
    ...
    for (const product of productsToUpdate) {
      ...
      if (newPrice < Number(product.cost_price) && newPrice < Number(product.price)) {
        // Ajuste para não subir preço abaixo do custo se já estiver assim
        logger.warn(
          `[SmartPricing] Skipped ${product.sku}: New price ${newPrice} below cost ${product.cost_price}. Product will not be sold at a loss.`,
        );
        continue;
      }
  ...
  async updateProductPrice(
    ...
    try {
      ...
      // Atualiza Produto
      await client.query(
        'UPDATE product_variations SET price = $1, updated_at = NOW() WHERE id = $2',
        [newPrice, variationId],
      );
      ...
      logger.info(
        `[SmartPricing] Updated variation ${variationId} from ${oldPrice} to ${newPrice}`,
      );
    } catch (error) {
