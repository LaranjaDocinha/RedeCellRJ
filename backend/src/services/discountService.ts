...
class DiscountService {
  private async getClient(dbClient?: PoolClient): Promise<PoolClient> {
    return dbClient || (await pool.connect());
  }

  async getAllDiscounts(dbClient?: PoolClient): Promise<Discount[]> {
  ...
  async getDiscountById(id: number, dbClient?: PoolClient): Promise<Discount | undefined> {
    const client = await this.getClient(dbClient);
    try {
  ...
  async createDiscount(payload: CreateDiscountPayload, dbClient?: PoolClient): Promise<Discount> {
    const client = await this.getClient(dbClient);
    try {
      const { name, type, value, start_date, end_date, min_purchase_amount, max_uses, is_active } =
        payload;
  ...
  async deleteDiscount(id: number, dbClient?: PoolClient): Promise<boolean> {
    const client = await this.getClient(dbClient);
    try {
    ...
    try {
      ...
      for (const discount of discounts) {
        ...
        if (discount.type === 'percentage') {
          savings = currentAmount * discount.value; // value is 0.10 for 10%
        } else {
