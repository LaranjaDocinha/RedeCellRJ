...
interface DashboardFilters {
  ...
  salesperson?: string; // ID do vendedor
  product?: string; // ID do produto
  region?: string; // Nome da região
  comparePeriod?: string; // Novo campo para o período de comparação
}

// Função auxiliar para construir a cláusula WHERE baseada em um *único* conjunto de parâmetros de período e filtros
function buildWhereClauseForPeriod(currentPeriod: string, currentStartDate: string | undefined, currentEndDate: string | undefined, filters: Omit<DashboardFilters, 'period' | 'startDate' | 'endDate' | 'comparePeriod'>, tableAlias: string = 's') {
...
function calculateComparisonPeriod(period: string, startDate: string | undefined, endDate: string | undefined, comparePeriod: string) {
  let compPeriod = { period: '', startDate: undefined as string | undefined, endDate: undefined as string | undefined };
  ...
  const moment = (date?: string) => {
    ...
    return {
      ...
      diff: (otherMoment: any, unit: 'days') => {
  ...
  const getEndOfMonth = (m: any) => moment(getStartOfMonth(m).add(1, 'month').subtract(1, 'day').format('YYYY-MM-DD'));
...
export const dashboardService = {
  async getTotalSalesAmount(filters: DashboardFilters = {}) {
    ...
    const { rows: mainRows } = await pool.query(
      `SELECT COALESCE(SUM(total_amount), 0) AS total_sales FROM sales s ${mainWhereClause};`,
      mainParams
    );
    ...
    if (comparePeriod && comparePeriod !== 'Nenhum') {
      ...
      if (compPeriod.period || (compPeriod.startDate && compPeriod.endDate)) {
        ...
        const { rows: compRows } = await pool.query(
          `SELECT COALESCE(SUM(total_amount), 0) AS total_sales FROM sales s ${compWhereClause};`,
          compParams
        );
  ...
  async getSlowMovingProducts(filters: DashboardFilters = {}, days: number = 90) {
    ...
    const fetchSlowMovingForPeriod = async (currentPeriod: string, currentStartDate: string | undefined, currentEndDate: string | undefined) => {
  ...
  async getSalesForecast(filters: DashboardFilters = {}) {
    ...
    const fetchSalesForecastForPeriod = async (currentPeriod: string, currentStartDate: string | undefined, currentEndDate: string | undefined) => {
      const { whereClause, params } = buildWhereClauseForPeriod(currentPeriod, currentStartDate, currentEndDate, { salesperson, product, region }, 's');
  ...
  async getAverageTicketBySalesperson(filters: DashboardFilters = {}) {
    ...
    const fetchAverageTicketForPeriod = async (currentPeriod: string, currentStartDate: string | undefined, currentEndDate: string | undefined) => {
      const { whereClause, params } = buildWhereClauseForPeriod(currentPeriod, currentStartDate, currentEndDate, { salesperson, product, region }, 's');
      const { rows } = await pool.query(
        `SELECT
          u.name as user_name,
