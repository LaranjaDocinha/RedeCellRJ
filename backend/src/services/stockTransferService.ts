...
export const stockTransferService = {
  async requestTransfer(transferData: Omit<StockTransfer, 'id' | 'status' | 'createdAt' | 'updatedAt' | 'approvedBy'>): Promise<StockTransfer> {
    const { productId, variationId, fromBranchId, toBranchId, quantity, requestedBy } = transferData;
    const pool = getPool();
    ...
    const productBranchRes = await pool.query(
      'SELECT branch_id FROM products WHERE id = $1',
      [productId]
    );

    if (productBranchRes.rows[0]?.branch_id !== fromBranchId) {
      throw new AppError(`Product ${productId} is not associated with source branch ${fromBranchId}.`, 400);
    }

    const availableStock = currentStockRes.rows[0].stock_quantity;
    if (availableStock < quantity) {
      throw new AppError(`Insufficient stock (${availableStock}) in source branch for transfer of ${quantity}.`, 400);
    }
  ...
  async approveTransfer(transferId: number, approvedBy: number): Promise<StockTransfer> {
    ...
    try {
      ...
      const targetVariationRes = await pool.query(
  ...
  async getPendingTransfers(): Promise<StockTransfer[]> {
    const pool = getPool();
    const result = await pool.query('SELECT * FROM stock_transfers WHERE status = $1 ORDER BY created_at DESC', ['pending']);
    return result.rows;
  },
