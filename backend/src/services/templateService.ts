...
class TemplateService {
  ...
  async getTemplateByName(name: string) { // Mantido para compatibilidade, mas getTemplateById é preferível para IDs
    const result = await pool.query('SELECT id, name, type, subject, content FROM templates WHERE name = $1', [name]);
    return result.rows[0];
  }

  async createTemplate(name: string, type: 'email' | 'sms', subject: string | null, content: string) {
  ...
  async updateTemplate(id: string, name: string, type: 'email' | 'sms', subject: string | null, content: string) {
    const result = await pool.query(
  ...
  async renderTemplate(content: string, context: any) { // Alterado para aceitar content diretamente
