...
interface CustomerJourney {
  ...
  action_type: 'email' | 'whatsapp_message' | 'push_notification';
  template_id: string;
  delay_days: number;
  is_active: boolean;
  created_at: Date;
...
export const customerJourneyService = {
  async getAllJourneys(): Promise<CustomerJourney[]> {
  ...
  async getActiveJourneys(): Promise<CustomerJourney[]> {
    const result = await pool.query('SELECT * FROM customer_journeys WHERE is_active = TRUE');
    return result.rows;
  },

  async processCustomerJourneys(): Promise<void> {
    ...
    for (const journey of activeJourneys) {
      ...
      const customersToProcess = await pool.query(
