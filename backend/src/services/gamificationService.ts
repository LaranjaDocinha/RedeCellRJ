...
export const awardBadge = async (userId: string, badgeId: number) => {
  const pool = getPool();
  try {
    await pool.query(
...
export const createChallenge = async (
  title: string,
  description: string,
  metric: string, // Ex: 'sales_volume', 'repairs_completed', 'customer_satisfaction'
  targetValue: number,
  rewardXp: number,
...
export const updateChallengeProgress = async (userId: string, metric: string, value: number) => {
  const pool = getPool();
