...
// Define the structure for supplier data when creating/updating a part
interface PartSupplierInput {
  supplier_id: number;
  cost: number;
...
// Define the structure for the part data payload
interface PartDataPayload extends Omit<Part, 'id' | 'created_at' | 'updated_at' | 'suppliers'> {
  suppliers?: PartSupplierInput[];
}

export const createPart = async (partData: PartDataPayload): Promise<Part> => {
  ...
  const partResult = await pool.query(
    ...
    [name, sku, description, stock_quantity],
  );
  ...
  return getPartById(newPart.id) as Promise<Part>;
};
...
export const updatePart = async (
  id: number,
  partData: Partial<PartDataPayload>,
): Promise<Part | null> => {
  const { name, sku, description, stock_quantity, suppliers } = partData;
  ...
  try {
    ...
    const partResult = await client.query(
      'UPDATE parts SET name = COALESCE($1, name), sku = COALESCE($2, sku), description = COALESCE($3, description), stock_quantity = COALESCE($4, stock_quantity), updated_at = current_timestamp WHERE id = $5 RETURNING *',
      [name, sku, description, stock_quantity, id],
    );
