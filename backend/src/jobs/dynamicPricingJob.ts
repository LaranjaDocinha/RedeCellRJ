...
const applyDynamicPricing = async () => {
  ...
  try {
    ...
    for (const product of productsToUpdate.rows) {
      ...
      await client.query(
        'INSERT INTO product_price_history (product_variation_id, old_price, new_price, change_reason) VALUES ($1, $2, $3, $4)',
        [product.id, oldPrice, newPrice, 'Precificação Dinâmica (30+ dias)'],
      );

      console.log(
        `Updated price for ${product.name} (ID: ${product.id}) from ${oldPrice} to ${newPrice}`,
      );
    }
