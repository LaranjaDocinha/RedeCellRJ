...
interface PartParams extends Request {
  params: {
    partId: string;
    supplierId?: string;
  };
...
partSuppliersRouter.get('/', async (req: PartParams, res, next) => {
  try {
...
partSuppliersRouter.post('/', validate(partSupplierSchema), async (req: PartParams, res, next) => {
  try {
    const { partId } = req.params;
...
partSuppliersRouter.put(
  '/:supplierId',
  validate(updatePartSupplierSchema),
  async (req: PartParams, res, next) => {
    try {
      const { partId, supplierId } = req.params;
      const updatedAssociation = await partSupplierService.updateSupplierForPart(
        parseInt(partId),
        parseInt(supplierId!),
        req.body,
      );
...
partSuppliersRouter.delete('/:supplierId', async (req: PartParams, res, next) => {
  try {
    const { partId, supplierId } = req.params;
    const deleted = await partSupplierService.removeSupplierFromPart(
      parseInt(partId),
      parseInt(supplierId!),
    );
    if (!deleted) {
