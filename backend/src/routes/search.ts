...
router.get(
  ...
  authMiddleware.authorize('read', 'Search'), // Assumindo que a autorização 'Search' é suficiente
  validateQuery(searchQuerySchema),
  async (req, res, next) => {
    try {
      const { q, entityType } = req.query; // Extrair entityType
      const results = await searchService.performSearch(q as string, entityType as any);
      res.json(results);
    } catch (error) {
      next(error);
    }
...
router.get(
  '/suggestions',
  authMiddleware.authenticate,
  authMiddleware.authorize('read', 'Search'), // Mesma autorização para sugestões
  validateQuery(suggestionsQuerySchema),
  async (req, res, next) => {
    try {
      const { q, entityType } = req.query; // Extrair entityType
      const suggestions = await searchService.getSuggestions(q as string, entityType as any);
