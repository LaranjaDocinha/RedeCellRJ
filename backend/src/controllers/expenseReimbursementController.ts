...
export const getRequests = async (req: Request, res: Response) => {
  try {
    ...
    const requests = await expenseReimbursementService.getRequests(
      status as string,
...
export const approveRequest = async (req: Request, res: Response) => {
  try {
    ...
    const request = await expenseReimbursementService.approveRequest(
...
export const rejectRequest = async (req: Request, res: Response) => {
  try {
    ...
    const request = await expenseReimbursementService.rejectRequest(
      parseInt(req.params.id, 10),
      String(reviewerId),
    );
    if (request) {
