...
describe('dynamicPricingService', () => {
    ...
    describe('getSuggestedUsedProductPrice', () => {
        it('should return null if product is not found or not a used product', async () => {
            ...
            expect(mockedDb.default.query).toHaveBeenCalledWith('SELECT p.acquisition_date, p.condition, p.created_at, pv.price as new_price FROM products p JOIN product_variations pv ON p.id = pv.product_id WHERE p.id = $1 AND p.is_used = TRUE LIMIT 1', [1]);
        });
