...
describe('IpWhitelistService', () => {
  ...
  describe('createEntry', () => {
    ...
    it('should throw error if IP exists', async () => {
      ...
      await expect(ipWhitelistService.createEntry({ ip_address: '1.1.1.1' })).rejects.toThrow(
        'IP address 1.1.1.1 already exists in whitelist.',
      );
    });
    ...
    it('should throw generic error on failure', async () => {
      ...
      await expect(ipWhitelistService.createEntry({ ip_address: '1.1.1.1' })).rejects.toThrow(
        'Failed to create IP whitelist entry.',
      );
    });
  });
  ...
  describe('updateEntry', () => {
    ...
    it('should throw NotFoundError if update affects no rows', async () => {
      ...
      await expect(ipWhitelistService.updateEntry(1, { is_active: false })).rejects.toThrow(
        NotFoundError,
      );
    });
    ...
    it('should return existing if no fields to update', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM ip_whitelist'),
        [1],
      );
    });
    ...
    it('should throw NotFoundError if entry not found and no fields to update', async () => {
      (pool.query as any).mockResolvedValue({ rows: [] });
      await expect(ipWhitelistService.updateEntry(1, {})).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('getActiveWhitelistedIps', () => {
    it('should return list of IPs', async () => {
      (pool.query as any).mockResolvedValue({
        rows: [{ ip_address: '1.2.3.4' }, { ip_address: '5.6.7.8' }],
      });
