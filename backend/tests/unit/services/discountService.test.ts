...
describe('DiscountService', () => {
  ...
  describe('createDiscount', () => {
    it('should create a discount', async () => {
      const payload = { name: 'New', type: 'percentage' as const, value: 0.1, start_date: '2023-01-01' };
    ...
    it('should throw AppError on unique violation', async () => {
      ...
      await expect(discountService.createDiscount({} as any))
        .rejects.toThrow('Discount with this name already exists');
    });
  });
  ...
  describe('updateDiscount', () => {
      ...
      it('should return existing discount if no fields to update', async () => {
          ...
          expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('SELECT * FROM discounts'), [1]);
      });
  });
  ...
  describe('applyDiscount', () => {
      it('should apply valid percentage discount', async () => {
          mockClient.query.mockResolvedValueOnce({ rows: [{ id: 1, is_active: true, type: 'percentage', value: 0.2 }] });
          const result = await discountService.applyDiscount(1, 100);
          expect(result).toBe(80);
          expect(mockClient.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE discounts SET uses_count'), [1]);
      });
      ...
      it('should throw error if discount is not applicable', async () => {
          ...
          await expect(discountService.applyDiscount(1, 100)).rejects.toThrow('Discount not applicable');
      });
  });
});
