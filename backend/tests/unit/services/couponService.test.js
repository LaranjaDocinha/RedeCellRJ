...
describe('couponService', () => {
    ...
    describe('createCoupon', () => {
        it('should create a new coupon', async () => {
            ...
            expect(mockedDb.default.query).toHaveBeenCalledWith('INSERT INTO coupons (code, type, value, start_date, end_date, min_purchase_amount, max_uses, is_active) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *', [
                newCouponData.code,
                newCouponData.type,
                newCouponData.value,
    ...
    describe('updateCoupon', () => {
        it('should update an existing coupon', async () => {
            ...
            expect(mockedDb.default.query).toHaveBeenCalledWith('UPDATE coupons SET value = $1, updated_at = current_timestamp WHERE code = $2 RETURNING *', [30, 'SUMMER20']);
        });
        it('should return undefined if no coupon is found for the given code', async () => {
            mockedDb.default.query.mockResolvedValueOnce({ rows: [] });
    ...
    describe('deleteCoupon', () => {
        it('should return true when a coupon is deleted successfully', async () => {
            ...
            expect(mockedDb.default.query).toHaveBeenCalledWith('DELETE FROM coupons WHERE id = $1 RETURNING id', [1]);
        });
    ...
    describe('deleteCouponByCode', () => {
        it('should return true when a coupon is deleted successfully', async () => {
            ...
            expect(result).toBe(true);
            expect(mockedDb.default.query).toHaveBeenCalledWith('DELETE FROM coupons WHERE code = $1 RETURNING id', ['SUMMER20']);
        });
    ...
    describe('applyCoupon', () => {
        ...
        it('should apply a valid percentage coupon', async () => {
            ...
            expect(mockedDb.default.query).toHaveBeenCalledWith('UPDATE coupons SET uses_count = uses_count + 1 WHERE code = $1', ['VALID20']);
        });
        ...
        it('should throw an error for an invalid coupon code', async () => {
            mockedDb.default.query.mockResolvedValueOnce({ rows: [] });
            await expect(couponService.applyCoupon('INVALID', 100)).rejects.toThrow('Coupon not applicable');
        });
        it('should throw an error for an expired coupon', async () => {
            ...
            await expect(couponService.applyCoupon(validCoupon.code, 100)).rejects.toThrow('Coupon not applicable');
        });
