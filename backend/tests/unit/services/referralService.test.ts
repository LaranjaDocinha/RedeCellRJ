...
describe('ReferralService', () => {
  ...
  describe('generateReferralCode', () => {
    it('should generate a referral code and save it to the database', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        'INSERT INTO referrals (referrer_customer_id, referral_code) VALUES ($1, $2)',
        [customerId, 'TESTCODE123'],
      );
    });
  });
  ...
  describe('applyReferralCode', () => {
    it('should apply a referral code to a new customer', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        'UPDATE referrals SET referred_customer_id = $1 WHERE referral_code = $2 AND referred_customer_id IS NULL RETURNING id',
        [newCustomerId, referralCode],
      );
    });
  ...
  describe('completeReferral', () => {
    it('should complete a referral, update status, and create coupons', async () => {
      ...
      vi.mocked(couponService.createCoupon).mockResolvedValue({} as any); // Mock coupon creation

      await referralService.completeReferral(mockReferral.referred_customer_id);

      // Check if referral was fetched
      expect(mockClient.query).toHaveBeenCalledWith(
        'SELECT * FROM referrals WHERE referred_customer_id = $1 AND status = $2',
        [mockReferral.referred_customer_id, 'pending'],
      );

      // Check if referral status was updated
      expect(mockClient.query).toHaveBeenCalledWith(
        'UPDATE referrals SET status = $1, completed_at = NOW() WHERE id = $2',
        ['completed', mockReferral.id],
      );
  ...
  describe('getReferralHistory', () => {
    it('should return the referral history for a customer', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        'SELECT * FROM referrals WHERE referrer_customer_id = $1',
        [customerId],
      );
    });
  });
