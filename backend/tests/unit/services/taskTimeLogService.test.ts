...
describe('TaskTimeLogService', () => {
  ...
  describe('startTimer', () => {
    it('should start timer if no active timer exists for task', async () => {
      ...
      const newLog = { id: 1, user_id: userId, service_order_id: serviceOrderId, start_time: new Date() };
    ...
    it('should throw error if timer already running', async () => {
      ...
      await expect(taskTimeLogService.startTimer(userId, serviceOrderId)).rejects.toThrow('Timer is already running for this task.');
    });
  });
  ...
  describe('stopTimer', () => {
    ...
    it('should throw error if no active timer found', async () => {
      ...
      await expect(taskTimeLogService.stopTimer(userId, serviceOrderId)).rejects.toThrow('No active timer found for this task.');
    });
  });
