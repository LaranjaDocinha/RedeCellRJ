...
describe('CycleCountService', () => {
  ...
  describe('createCycleCount', () => {
    it('should create cycle count and items', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO cycle_count_items'),
        expect.arrayContaining([1, 101, 12, 10, 2, undefined]),
      );
    });
    ...
    it('should rollback on error during creation', async () => {
      ...
      await expect(cycleCountService.createCycleCount({ items: [] } as any)).rejects.toThrow(
        'Fail',
      );
  ...
  describe('updateCycleCount', () => {
    it('should update status and notes', async () => {
      ...
      const result = await cycleCountService.updateCycleCount(1, {
        status: 'in_progress',
        notes: 'New',
      });
      expect(result.id).toBe(1);
    ...
    it('should adjust stock when status is completed', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining(
          'UPDATE product_variations SET stock_quantity = stock_quantity + $1',
        ),
        [-2, 101],
      );
    });
