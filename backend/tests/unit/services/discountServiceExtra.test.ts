...
describe('DiscountService Extra Coverage', () => {
  ...
  describe('applyDiscount (Applicability Branches)', () => {
    it('should throw error if discount is expired', async () => {
        ...
        mockClient.query.mockResolvedValue({ rows: [{ id: 1, is_active: true, end_date: pastDate }] });
        
        await expect(discountService.applyDiscount(1, 100))
            .rejects.toThrow('Discount not applicable');
    });

    it('should throw error if max uses reached', async () => {
        mockClient.query.mockResolvedValue({ rows: [{ id: 1, is_active: true, max_uses: 5, uses_count: 5 }] });
    ...
    it('should throw error if min purchase amount not met', async () => {
        mockClient.query.mockResolvedValue({ rows: [{ id: 1, is_active: true, min_purchase_amount: 200 }] });
    ...
    it('should apply fixed amount discount correctly', async () => {
        mockClient.query.mockResolvedValueOnce({ rows: [{ id: 1, is_active: true, type: 'fixed_amount', value: 30 }] });
