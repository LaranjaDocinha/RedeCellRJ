...
describe('TimeClockService', () => {
  ...
  describe('clockIn', () => {
    it('should clock in successfully if no open entry exists', async () => {
      ...
      const newEntry = { id: 1, user_id: userId, branch_id: branchId, clock_in_time: new Date(), clock_out_time: null };
    ...
    it('should throw error if user is already clocked in', async () => {
      ...
      await expect(timeClockService.clockIn(userId, branchId)).rejects.toThrow('User is already clocked in.');
  ...
  describe('clockOut', () => {
    it('should clock out successfully if open entry exists', async () => {
      const userId = 'user-123';
      const updatedEntry = { id: 1, user_id: userId, clock_in_time: new Date(), clock_out_time: new Date() };
    ...
    it('should throw error if no open clock-in found', async () => {
      ...
      await expect(timeClockService.clockOut(userId)).rejects.toThrow('No open clock-in found for this user.');
    });
  });
