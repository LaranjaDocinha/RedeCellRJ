...
describe('accountingIntegrationService', () => {
  it('syncSales should return simulated success', async () => {
    ...
    expect(result).toEqual({ success: true, message: 'Sales data sent to accounting software (simulated).' });
    expect(consoleSpy).toHaveBeenCalledWith('Simulating sales synchronization with accounting software:', [{ id: 1, amount: 100 }]);
  ...
  it('syncExpenses should return simulated success', async () => {
    ...
    expect(result).toEqual({ success: true, message: 'Expenses data sent to accounting software (simulated).' });
    expect(consoleSpy).toHaveBeenCalledWith('Simulating expenses synchronization with accounting software:', [{ id: 1, amount: 50 }]);
    consoleSpy.mockRestore();
  });
