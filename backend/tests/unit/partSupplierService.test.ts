...
describe('partSupplierService', () => {
  ...
  describe('addSupplierToPart', () => {
    it('should add a supplier to a part', async () => {
      ...
      expect(mockedDb.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO part_suppliers'),
        [1, 1, 100, 5, undefined],
      );
    });
    ...
    it('should throw an error if the supplier is already associated with the part', async () => {
      ...
      await expect(partSupplierService.addSupplierToPart(mockPayload)).rejects.toThrow(
        new AppError('This supplier is already associated with this part', 409),
      );
    });
  });
  ...
  describe('updateSupplierForPart', () => {
    it('should update a supplier for a part', async () => {
      ...
      expect(result).toEqual(mockReturn);
      expect(mockedDb.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE part_suppliers'),
        [110, 4, 1, 1],
      );
    });
  });
