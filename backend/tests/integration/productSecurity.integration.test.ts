...
describe('Product API Security - Integration', () => {
  ...
  beforeAll(async () => {
    ...
    userNoPermissionToken = noPermissionRes.body.token;
  ...
  afterAll(async () => {
    ...
    for (const roleId of rolesToCleanup) {
      await pool.query('DELETE FROM role_permissions WHERE role_id = $1', [roleId]).catch(console.error);
    ...
    for (const productId of productsToCleanup) {
        ...
        await pool.query('DELETE FROM price_history WHERE variation_id IN (SELECT id FROM product_variations WHERE product_id = $1)', [productId]).catch(console.error);
        await pool.query('DELETE FROM branch_product_variations_stock WHERE product_variation_id IN (SELECT id FROM product_variations WHERE product_id = $1)', [productId]).catch(console.error);
        await pool.query('DELETE FROM product_variations WHERE product_id = $1', [productId]).catch(console.error);
  ...
  describe('POST /api/products - Authorization and Validation', () => {
    ...
    beforeEach(() => {
        validProductData = {
            ...
            variations: [
                { color: 'Blue', price: 100, stock_quantity: 5, low_stock_threshold: 1 },
            ],
        };
    ...
    it('should return 403 if user does not have "manage:Product" permission', async () => {
      ...
      expect(res.body.message).toEqual('You do not have permission to manage Product');
    });
    ...
    it('should return 422 for invalid product data', async () => {
      ...
      expect(res.body.errors).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({ path: 'variations.0.price', message: 'Preço deve ser positivo' }),
        ])
      );
    });
    ...
    it('should create product successfully with admin token and valid data', async () => {
      ...
      expect(res.body.name).toEqual(validProductData.name);
      productsToCleanup.push(res.body.id); // Adicionar para cleanup
    });
  });
  ...
  describe('PUT /api/products/:id - Authorization and Validation', () => {
    ...
    beforeEach(async () => {
        const pool = getPool(); // Obter o pool para cleanup antes de cada teste
        ...
        const res = await request(httpServer)
            ...
            .send({
                ...
                product_type: 'eletronic',
                variations: [
                    { color: 'Green', price: 50, stock_quantity: 10, low_stock_threshold: 1 },
                ],
            });
        ...
        updateProductData = {
            ...
            variations: [
                { id: res.body.variations[0].id, color: 'Blue', price: 120, stock_quantity: 8, low_stock_threshold: 1 },
            ],
        };
        ...
        invalidUpdateData = {
            name: '', // Nome vazio
            variations: [
                { id: res.body.variations[0].id, color: 'Orange', price: 0, stock_quantity: -5, low_stock_threshold: 1 }, // Preço inválido, estoque negativo
            ],
        };
    });
    ...
    it('should return 422 for invalid update data', async () => {
      ...
      expect(res.body.errors).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({ path: 'variations.0.price', message: 'Preço deve ser positivo' }),
          expect.objectContaining({ path: 'variations.0.stock_quantity', message: 'Estoque não pode ser negativo' }),
        ])
      );
    });
    ...
    it('should update product successfully with admin token and valid data', async () => {
      ...
      expect(res.body.name).toEqual(updateProductData.name);
    });
  });
});
