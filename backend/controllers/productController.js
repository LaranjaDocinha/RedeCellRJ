...
exports.getAllProducts = async (req, res) => {
  ...
  try {
    ...
    // Determine the next available placeholder index for LIMIT and OFFSET
    const limitOffsetPart = " ORDER BY p.created_at DESC LIMIT $" + (searchParams.length + 1) + " OFFSET $" + (searchParams.length + 2) + ";";
    baseQuery += limitOffsetPart;
