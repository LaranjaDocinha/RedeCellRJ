...
class StoreCreditService {
    async addCredit(customerId, amount, reason, relatedId) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            await client.query('UPDATE customers SET store_credit_balance = store_credit_balance + $1 WHERE id = $2', [amount, customerId]);
    ...
    async useCredit(customerId, amount, reason, relatedId) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            ...
            await client.query('UPDATE customers SET store_credit_balance = store_credit_balance - $1 WHERE id = $2', [amount, customerId]);
            await client.query('INSERT INTO store_credit_transactions (customer_id, amount, type, reason, related_id) VALUES ($1, $2, $3, $4, $5)', [customerId, -amount, 'debit', reason, relatedId]);
            await client.query('COMMIT');
        }
        catch (error) {
            await client.query('ROLLBACK');
