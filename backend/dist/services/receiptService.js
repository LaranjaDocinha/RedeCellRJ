...
class ReceiptService {
    ...
    async generateReceipt(saleId) {
        const pool = getPool();
        const saleRes = await pool.query(`SELECT
        s.id AS sale_id,
        s.sale_date,
        s.total_amount,
      ...
      WHERE s.id = $1`, [saleId]);
        ...
        const itemsRes = await pool.query(`SELECT
      ...
      WHERE si.sale_id = $1`, [saleId]);
        ...
        const paymentsRes = await pool.query(`SELECT
        payment_method,
        amount
      FROM sale_payments
...
`;
    ...
    async generateFiscalNote(saleId) {
        const pool = getPool();
        const saleRes = await pool.query(`SELECT
        s.id AS sale_id,
        s.sale_date,
...
`;
    ...
    async sendDocumentByEmail(to, subject, htmlContent, textContent) {
        try {
