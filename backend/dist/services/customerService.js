...
class CustomerService {
    ...
    async addStoreCredit(customerId, amount, relatedId, client) {
        const result = await client.query('UPDATE customers SET store_credit_balance = store_credit_balance + $1 WHERE id = $2 RETURNING *', [amount, customerId]);
        if (result.rows.length > 0) {
            await client.query('INSERT INTO store_credit_transactions (customer_id, amount, type, reason, related_id) VALUES ($1, $2, $3, $4, $5)', [customerId, amount, 'credit', 'Manual adjustment', relatedId]);
        }
        return result.rows[0];
    }
