...
export const saleService = {
    async createSale({ customerId, items, payments, userId, branchId = 1, client: externalClient, }) {
        console.log('Received userId:', userId);
        console.log('[createSale] saleData:', { customerId, items, payments, userId, branchId });
        ...
        try {
            ...
            for (const item of items) {
                ...
                const { rows: variationRows } = await client.query('SELECT pv.price, ps.quantity as stock_quantity, pv.cost_price FROM product_variations pv JOIN product_stock ps ON pv.id = ps.product_variant_id WHERE pv.id = $1 AND ps.branch_id = $2 FOR UPDATE', [item.variation_id, branchId]);
            ...
            const { rows: saleRows } = await client.query('INSERT INTO sales (user_id, customer_id, total_amount) VALUES ($1, $2, $3) RETURNING id, sale_date', [userId, customerId, totalAmount]);
