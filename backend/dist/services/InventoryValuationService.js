...
class InventoryValuationService {
    async getVariationCost(variationId) {
    ...
    async calculateFIFO() {
        ...
        const stockVariations = await pool.query('SELECT id, stock_quantity FROM product_variations WHERE stock_quantity > 0');
        let totalInventoryValue = 0;
        for (const variation of stockVariations.rows) {
            let stockToValue = variation.stock_quantity;
            const purchaseLayers = await pool.query(`SELECT unit_cost, quantity_remaining
         FROM inventory_movements
         WHERE product_variation_id = $1
           AND quantity_change > 0
           AND quantity_remaining > 0
         ORDER BY created_at ASC`, [variation.id]);
