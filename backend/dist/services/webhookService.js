...
export const createWebhook = async (eventType, callbackUrl, secret) => {
    const result = await getPool().query('INSERT INTO webhooks (event_type, callback_url, secret) VALUES ($1, $2, $3) RETURNING *', [eventType, callbackUrl, secret]);
    return result.rows[0];
};
export const updateWebhookStatus = async (id, isActive) => {
    const result = await getPool().query('UPDATE webhooks SET is_active = $1 WHERE id = $2 RETURNING *', [isActive, id]);
    return result.rows[0];
};
...
export const triggerWebhook = async (eventType, payload) => {
    const webhooks = await getPool().query('SELECT * FROM webhooks WHERE event_type = $1 AND is_active = TRUE', [eventType]);
