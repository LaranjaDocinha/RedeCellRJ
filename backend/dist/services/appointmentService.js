...
export const createAppointment = async (customerId, serviceType, appointmentDate, notes) => {
    const result = await getPool().query('INSERT INTO appointments (customer_id, service_type, appointment_date, notes) VALUES ($1, $2, $3, $4) RETURNING *', [customerId, serviceType, appointmentDate, notes]);
    return result.rows[0];
};
export const updateAppointmentStatus = async (id, status) => {
    const result = await getPool().query('UPDATE appointments SET status = $1 WHERE id = $2 RETURNING *', [status, id]);
    return result.rows[0];
};
