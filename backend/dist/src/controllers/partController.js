...
export const addSupplierToPart = async (req, res) => {
    try {
...
export const updateSupplierForPart = async (req, res) => {
    try {
        const partId = parseInt(req.params.partId, 10);
...
export const removeSupplierFromPart = async (req, res) => {
    try {
        const partId = parseInt(req.params.partId, 10);
        const supplierId = parseInt(req.params.supplierId, 10);
...
export const searchParts = async (req, res) => {
    try {
        ...
        const parts = await partService.searchParts(searchTerm, barcode, sku);
        res.status(200).json(parts);
    }
    catch (error) {
        if (error instanceof AppError) {
