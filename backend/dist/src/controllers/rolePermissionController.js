...
export const createRole = async (req, res, next) => {
    try {
        ...
        const id = uuidv4();
        const result = await pool.query('INSERT INTO roles (id, name, description) VALUES ($1, $2, $3) RETURNING *', [id, name, description]);
...
export const updateRole = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { name, description } = req.body;
        const result = await pool.query('UPDATE roles SET name = $1, description = $2, updated_at = current_timestamp WHERE id = $3 RETURNING *', [name, description, id]);
...
export const assignPermissionToRole = async (req, res, next) => {
    try {
        ...
        await pool.query('INSERT INTO role_permissions (role_id, permission_id) VALUES ($1, $2) ON CONFLICT (role_id, permission_id) DO NOTHING', [roleId, permissionId]);
