...
export const generateAndSaveContract = async (req, res) => {
    try {
...
export const getSaleContractById = async (req, res) => {
    try {
...
export const getSaleContractsBySaleId = async (req, res) => {
    try {
        const saleId = parseInt(req.params.saleId, 10);
        if (isNaN(saleId)) {
            return res.status(400).json({ message: 'ID da Venda inválido.' });
...
export const signSaleContract = async (req, res) => {
    try {
        ...
        const updatedContract = await contractService.updateSaleContractSignature(contractId, validatedData.signature_image_url);
        if (!updatedContract) {
            return res.status(404).json({ message: 'Contrato não encontrado.' });
        }
