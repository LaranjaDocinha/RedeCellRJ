...
export const quoteService = {
    async createQuote(payload) {
        ...
        try {
            ...
            const quoteResult = await client.query('INSERT INTO quotes (customer_id, user_id, valid_until, notes) VALUES ($1, $2, $3, $4) RETURNING *', [customer_id, user_id, valid_until, notes]);
            const newQuote = quoteResult.rows[0];
            for (const item of items) {
                await client.query('INSERT INTO quote_items (quote_id, product_id, variation_id, quantity, unit_price) VALUES ($1, $2, $3, $4, $5)', [newQuote.id, item.product_id, item.variation_id, item.quantity, item.unit_price]);
            }
    ...
    async updateQuote(id, payload) {
        ...
        try {
            ...
            if (fields.length > 0) {
                values.push(id);
                await client.query(`UPDATE quotes SET ${fields.join(', ')}, updated_at = current_timestamp WHERE id = $${paramIndex} RETURNING *`, values);
            }
            if (items !== undefined) {
                await client.query('DELETE FROM quote_items WHERE quote_id = $1', [id]);
                for (const item of items) {
                    await client.query('INSERT INTO quote_items (quote_id, product_id, variation_id, quantity, unit_price) VALUES ($1, $2, $3, $4, $5)', [id, item.product_id, item.variation_id, item.quantity, item.unit_price]);
                }
            }
