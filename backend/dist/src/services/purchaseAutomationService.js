 ...
 */
export const getPartStockQuantity = async (partId) => {
 ...
 */
export const createPurchaseOrderForParts = async (purchaseOrderData, parts) => {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const { supplier_id, status, expected_delivery_date, notes } = purchaseOrderData;
        const poRes = await client.query('INSERT INTO purchase_orders (supplier_id, status, expected_delivery_date, notes) VALUES ($1, $2, $3, $4) RETURNING *', [supplier_id, status, expected_delivery_date, notes]);
        const newPurchaseOrder = poRes.rows[0];
        for (const part of parts) {
            const { part_id, quantity, unit_price } = part;
            await client.query('INSERT INTO purchase_order_parts (purchase_order_id, part_id, quantity, unit_price) VALUES ($1, $2, $3, $4)', [newPurchaseOrder.id, part_id, quantity, unit_price]);
        }
 ...
 */
export const checkAndRequestPartsForServiceOrder = async (serviceOrderId) => {
    // 1. Obter todas as peças necessárias para a ordem de serviço
    const serviceOrderPartsRes = await pool.query(`SELECT soi.part_id, soi.quantity, p.stock_quantity, p.supplier_id, p.cost_price
     FROM service_order_items soi
     JOIN parts p ON soi.part_id = p.id
     WHERE soi.service_order_id = $1 AND soi.part_id IS NOT NULL`, [serviceOrderId]);
    const partsToOrder = {};
