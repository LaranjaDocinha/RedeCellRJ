...
export const getServiceOrderById = async (id) => {
    ...
    const itemsResult = await query('SELECT * FROM service_order_items WHERE service_order_id = $1', [id]);
    order.items = itemsResult.rows;
    const attachmentsResult = await query('SELECT * FROM service_order_attachments WHERE service_order_id = $1', [id]);
...
export const addOrderItem = async (service_order_id, itemData) => {
    ...
    const result = await query('INSERT INTO service_order_items (service_order_id, part_id, service_description, quantity, unit_price) VALUES ($1, $2, $3, $4, $5) RETURNING *', [service_order_id, part_id, service_description, quantity, unit_price]);
...
export const updateServiceOrder = async (id, orderData) => {
    ...
    const result = await query('UPDATE service_orders SET technical_report = $1, budget_value = $2, updated_at = current_timestamp WHERE id = $3 RETURNING *', [technical_report, budget_value, id]);
...
export const addComment = async (serviceOrderId, userId, commentText) => {
    const result = await query('INSERT INTO service_order_comments (service_order_id, user_id, comment_text) VALUES ($1, $2, $3) RETURNING *', [serviceOrderId, userId, commentText]);
    return result.rows[0];
};
export const getComments = async (serviceOrderId) => {
    const result = await query('SELECT soc.*, u.name as user_name FROM service_order_comments soc JOIN users u ON soc.user_id = u.id WHERE service_order_id = $1 ORDER BY created_at ASC', [serviceOrderId]);
    return result.rows;
};
export const addServiceOrderAttachment = async (serviceOrderId, filePath, fileType, description, uploadedByUserId) => {
    const res = await query('INSERT INTO service_order_attachments (service_order_id, file_path, file_type, description, uploaded_by_user_id) VALUES ($1, $2, $3, $4, $5) RETURNING *', [serviceOrderId, filePath, fileType, description, uploadedByUserId]);
    return res.rows[0];
};
