...
class ReviewService {
    async getReviewsByProductId(productId) {
        const result = await pool.query('SELECT pr.*, u.email as user_email, u.name as user_name FROM product_reviews pr JOIN users u ON pr.user_id = u.id WHERE pr.product_id = $1 ORDER BY pr.created_at DESC', [productId]);
        return result.rows;
    }
    ...
    async createReview(payload) {
        ...
        try {
            ...
            const existingReview = await pool.query('SELECT id FROM product_reviews WHERE product_id = $1 AND user_id = $2', [product_id, user_id]);
            if (existingReview.rows.length > 0) {
                throw new AppError('You have already reviewed this product', 409);
            }
            const result = await pool.query('INSERT INTO product_reviews (product_id, user_id, rating, comment) VALUES ($1, $2, $3, $4) RETURNING *', [product_id, user_id, rating, comment]);
            return result.rows[0];
        }
        catch (error) {
    ...
    async updateReview(id, userId, payload) {
