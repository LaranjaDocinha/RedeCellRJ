...
export const createFranchise = async (name, address, contactPerson, contactEmail) => {
    const result = await getPool().query('INSERT INTO franchises (name, address, contact_person, contact_email) VALUES ($1, $2, $3, $4) RETURNING *', [name, address, contactPerson, contactEmail]);
    return result.rows[0];
};
export const updateFranchiseStatus = async (id, isActive) => {
    const result = await getPool().query('UPDATE franchises SET is_active = $1 WHERE id = $2 RETURNING *', [isActive, id]);
    return result.rows[0];
};
...
export const getFranchiseSettings = async (franchiseId) => {
