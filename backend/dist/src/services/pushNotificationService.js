...
if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY) {
    ...
    webPush.setVapidDetails('mailto:your_email@example.com', // Replace with your email
...
export const pushNotificationService = {
    ...
    async subscribeUser(userId, subscription) {
        try {
            ...
            const existing = await pool.query('SELECT id FROM user_push_subscriptions WHERE user_id = $1 AND (subscription->>\'endpoint\') = $2', [userId, subscription.endpoint]);
            if (existing.rows.length > 0) {
                return existing.rows[0]; // Already subscribed
            }
            const result = await pool.query('INSERT INTO user_push_subscriptions (user_id, subscription) VALUES ($1, $2) RETURNING *', [userId, subscription]);
    ...
    async unsubscribeUser(userId, endpoint) {
        const result = await pool.query('DELETE FROM user_push_subscriptions WHERE user_id = $1 AND (subscription->>\'endpoint\') = $2', [userId, endpoint]);
        return result.rowCount > 0;
    },
    async sendNotificationToUser(userId, title, body, icon, url) {
        const subscriptions = await pool.query('SELECT subscription FROM user_push_subscriptions WHERE user_id = $1', [userId]);
    ...
    async sendNotificationToAll(title, body, icon, url) {
        const subscriptions = await pool.query('SELECT subscription FROM user_push_subscriptions');
        const notificationPayload = JSON.stringify({
            title,
            body,
