...
export const stockPredictionService = {
     ...
     */
    async predictStockNeeds(lookbackDays = 90, predictionDays = 30) {
        const now = new Date();
        ...
        const salesDataRes = await pool.query(`SELECT
         pv.id as item_id,
       ...
       WHERE s.sale_date >= $1
       GROUP BY pv.id`, [lookbackDate.toISOString()]);
        // 2. Obter dados históricos de peças usadas em reparos
        const repairDataRes = await pool.query(`SELECT
         p.id as item_id,
         'part' as item_type,
       ...
       GROUP BY p.id`, [lookbackDate.toISOString()]);
        // Consolidar demanda histórica
        const historicalDemand = {}; // Key: "type_id"
        ...
        const allProductVariationsRes = await pool.query('SELECT id, product_id, stock_quantity, low_stock_threshold FROM product_variations');
        ...
        const allPartsRes = await pool.query('SELECT id, name, stock_quantity, low_stock_threshold FROM parts');
