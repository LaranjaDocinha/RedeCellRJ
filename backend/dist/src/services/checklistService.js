...
export const getChecklistTemplateWithItems = async (templateId) => {
    const templateRes = await pool.query('SELECT * FROM checklist_templates WHERE id = $1', [
        templateId,
    ]);
    if (templateRes.rows.length === 0)
        return null;
    const itemsRes = await pool.query('SELECT * FROM checklist_template_items WHERE template_id = $1 ORDER BY order_index, id', [templateId]);
    return { ...templateRes.rows[0], items: itemsRes.rows };
};
export const saveChecklistAnswers = async (serviceOrderId, userId, answers) => {
...
export const createChecklistTemplate = async (data) => {
    ...
    const res = await pool.query('INSERT INTO checklist_templates (name, description, type) VALUES ($1, $2, $3) RETURNING *', [name, description, type]);
    return res.rows[0];
};
...
export const updateChecklistTemplate = async (id, data) => {
    ...
    const res = await pool.query(`UPDATE checklist_templates SET ${fields}, updated_at = current_timestamp WHERE id = $1 RETURNING *`, [id, ...values]);
    return res.rows[0];
};
...
export const createChecklistTemplateItem = async (data) => {
    const { template_id, item_text, response_type, order_index } = data;
    const res = await pool.query('INSERT INTO checklist_template_items (template_id, item_text, response_type, order_index) VALUES ($1, $2, $3, $4) RETURNING *', [template_id, item_text, response_type, order_index]);
    return res.rows[0];
};
export const getChecklistTemplateItemById = async (id) => {
    const res = await pool.query('SELECT * FROM checklist_template_items WHERE id = $1', [id]);
    return res.rows[0];
};
export const getChecklistTemplateItemsByTemplateId = async (templateId) => {
    const res = await pool.query('SELECT * FROM checklist_template_items WHERE template_id = $1 ORDER BY order_index, id', [templateId]);
    return res.rows;
};
export const updateChecklistTemplateItem = async (id, data) => {
    const fields = Object.keys(data)
        .map((key, index) => `"${key}" = $${index + 2}`)
        .join(', ');
    const values = Object.values(data);
    const res = await pool.query(`UPDATE checklist_template_items SET ${fields}, updated_at = current_timestamp WHERE id = $1 RETURNING *`, [id, ...values]);
    return res.rows[0];
};
export const deleteChecklistTemplateItem = async (id) => {
