...
class ShiftReportService {
     ...
     */
    async getCurrentShiftReport(branchId) {
        const pool = getPool();
      ...
      FROM sales s
      WHERE s.sale_date >= $1 AND s.branch_id = $2`, [twentyFourHoursAgo, branchId]);
      ...
      ORDER BY amount DESC`, [twentyFourHoursAgo, branchId]);
      ...
      ORDER BY amount DESC`, [twentyFourHoursAgo, branchId]);
        ...
        const averageTransactionValue = total_transactions > 0 ? parseFloat(total_sales_amount) / parseInt(total_transactions) : 0;
        return {
