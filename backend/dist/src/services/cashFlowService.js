...
export const getCashFlowData = async (branchId, startDate, endDate) => {
    const client = await getPool().connect();
    try {
        let salesQuery = 'SELECT SUM(total_amount) as total_inflow FROM sales WHERE sale_date BETWEEN $1 AND $2';
        let expensesQuery = "SELECT SUM(amount) as total_outflow FROM expense_reimbursements WHERE status = 'approved' AND created_at BETWEEN $1 AND $2";
        let purchaseOrdersQuery = 'SELECT SUM(total_amount) as total_po_outflow FROM purchase_orders WHERE created_at BETWEEN $1 AND $2';
        ...
        const totalOutflow = (parseFloat(expensesRes.rows[0].total_outflow) || 0) +
    ...
    `;
        ...
        Array.from(allDates)
            .sort()
            .forEach((date) => {
            const inflow = dailyInflows.rows.find((r) => moment(r.date).format('YYYY-MM-DD') === date)
                ?.daily_inflow || 0;
            const expense = dailyExpenses.rows.find((r) => moment(r.date).format('YYYY-MM-DD') === date)
                ?.daily_outflow || 0;
            const poOutflow = dailyPurchaseOrders.rows.find((r) => moment(r.date).format('YYYY-MM-DD') === date)
                ?.daily_po_outflow || 0;
            cashFlowTrend.push({
                date,
