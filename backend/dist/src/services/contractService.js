...
export const contractService = {
     ...
     */
    async generateContractHtml(saleData) {
      ...
      <p>Itens:</p>
      <ul>
        ${saleData.items.map((item) => `<li>${item.quantity}x ${item.product_name} - R$ ${item.price_at_sale}</li>`).join('')}
      </ul>
    ...
    async createSaleContract(data) {
        ...
        const res = await pool.query('INSERT INTO sale_contracts (sale_id, contract_url, signature_image_url, signed_at) VALUES ($1, $2, $3, $4) RETURNING *', [sale_id, contract_url, signature_image_url, signed_at]);
        return res.rows[0];
    },
    /**
     ...
     */
    async getSaleContractById(contractId) {
     ...
     */
    async getSaleContractsBySaleId(saleId) {
        const res = await pool.query('SELECT * FROM sale_contracts WHERE sale_id = $1 ORDER BY created_at DESC', [saleId]);
        return res.rows;
    },
    /**
     ...
     */
    async updateSaleContractSignature(contractId, signatureImageUrl) {
        const res = await pool.query('UPDATE sale_contracts SET signature_image_url = $1, signed_at = current_timestamp, updated_at = current_timestamp WHERE id = $2 RETURNING *', [signatureImageUrl, contractId]);
        return res.rows[0];
    },
};
