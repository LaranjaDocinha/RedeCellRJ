...
export const saleService = {
    async createSale({ customerId, items, payments, userId, branchId = 1, client: externalClient, externalOrderId, marketplaceIntegrationId, }) {
        ...
        try {
            ...
            for (const item of items) {
                ...
                console.log(`[createSale] Searching for variation_id: ${item.variation_id} in branch: ${branchId}`);
                // Updated query to include is_serialized from products table
                const { rows: variationRows } = await client.query(`SELECT pv.price, ps.quantity as stock_quantity, pv.cost_price, p.is_serialized 
           FROM product_variations pv 
           JOIN product_stock ps ON pv.product_variant_id = ps.product_variant_id 
           JOIN products p ON pv.product_id = p.id
           WHERE pv.id = $1 AND ps.branch_id = $2 FOR UPDATE`, [item.variation_id, branchId]);
                console.log(`[createSale] Query result for variation ${item.variation_id}: ${JSON.stringify(variationRows)}`);
                ...
                if (variation.is_serialized) {
                    ...
                    for (const serial of item.serial_numbers) {
                        ...
                        const { rows: serialRows } = await client.query(`SELECT id, status FROM serialized_items WHERE serial_number = $1 AND product_variation_id = $2 AND branch_id = $3 FOR UPDATE`, [serial, item.variation_id, branchId]);
                        ...
                        await client.query(`UPDATE serialized_items SET status = 'sold', updated_at = NOW() WHERE id = $1`, [serialItem.id]);
                        // Log history (Manual insert since we are in a transaction and service doesn't support client injection yet)
                        await client.query('INSERT INTO serialized_items_history (serialized_item_id, action, old_status, new_status, user_id, details) VALUES ($1, $2, $3, $4, $5, $6)', [serialItem.id, 'sale', 'in_stock', 'sold', userId, JSON.stringify({ saleId: 'PENDING' })] // We'll update saleId later or ignore for now
                        );
                    }
                }
                else {
                ...
                console.log(`[createSale] item.unit_price: ${item.unit_price}, item.quantity: ${item.quantity}`);
                const itemTotalPrice = item.unit_price * item.quantity;
                totalAmount += itemTotalPrice;
                console.log(`[createSale] Item total: ${itemTotalPrice}, Running sale total: ${totalAmount}`);
                saleItems.push({
                ...
                console.log(`[createSale] Updating stock for variation ${item.variation_id} in branch ${branchId}`);
                await client.query('UPDATE product_stock SET quantity = quantity - $1 WHERE product_variant_id = $2 AND branch_id = $3', [item.quantity, item.variation_id, branchId]);
            }
            console.log(`[createSale] Inserting into sales table. UserID: ${userId}, CustomerID: ${customerId}, TotalAmount: ${totalAmount}`);
            const { rows: saleRows } = await client.query('INSERT INTO sales (user_id, customer_id, total_amount, external_order_id, marketplace_integration_id) VALUES ($1, $2, $3, $4, $5) RETURNING id, sale_date', [userId, customerId, totalAmount, externalOrderId, marketplaceIntegrationId]);
            ...
            for (const si of saleItems) {
                await client.query('INSERT INTO sale_items (sale_id, product_id, variation_id, quantity, unit_price, cost_price, total_price, metadata) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', [
                    newSale.id,
                    si.product_id,
                    si.variation_id,
                    si.quantity,
            ...
            for (const payment of payments) {
                ...
                await client.query('INSERT INTO sale_payments (sale_id, payment_method, amount, transaction_details) VALUES ($1, $2, $3, $4)', [
                    newSale.id,
                    payment.method,
                    payment.amount,
            ...
            try {
                await activityFeedService.createActivity(userId, branchId, 'sale', { saleId: newSale.id, totalAmount }, client);
            }
            catch (feedError) {
