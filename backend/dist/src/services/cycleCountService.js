...
export const cycleCountService = {
    async createCycleCount(payload) {
        ...
        try {
            ...
            const countResult = await client.query('INSERT INTO cycle_counts (counted_by_user_id, branch_id, notes) VALUES ($1, $2, $3) RETURNING *', [counted_by_user_id, branch_id, notes]);
            const newCycleCount = countResult.rows[0];
            for (const item of items) {
                // Obter a quantidade atual do sistema
                const systemStockRes = await client.query('SELECT stock_quantity FROM product_variations WHERE id = $1 AND branch_id = $2', [item.product_variation_id, branch_id]);
                const system_quantity = systemStockRes.rows[0]?.stock_quantity || 0;
                const discrepancy = item.counted_quantity - system_quantity;
                await client.query('INSERT INTO cycle_count_items (cycle_count_id, product_variation_id, counted_quantity, system_quantity, discrepancy, notes) VALUES ($1, $2, $3, $4, $5, $6)', [
                    newCycleCount.id,
    ...
    async getCycleCountById(id) {
        ...
        const itemsResult = await pool.query('SELECT * FROM cycle_count_items WHERE cycle_count_id = $1', [id]);
    ...
    async updateCycleCount(id, payload) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            ...
            if (fields.length > 0) {
                values.push(id);
                await client.query(`UPDATE cycle_counts SET ${fields.join(', ')}, updated_at = current_timestamp WHERE id = $${paramIndex} RETURNING *`, values);
            }
            if (items !== undefined) {
                ...
                for (const item of items) {
                    // Obter a quantidade atual do sistema
                    const systemStockRes = await client.query('SELECT stock_quantity FROM product_variations WHERE id = $1', // branch_id não é necessário aqui se a variação já está associada à filial
                    [item.product_variation_id]);
                    const system_quantity = systemStockRes.rows[0]?.stock_quantity || 0;
                    const discrepancy = item.counted_quantity - system_quantity;
                    await client.query('INSERT INTO cycle_count_items (cycle_count_id, product_variation_id, counted_quantity, system_quantity, discrepancy, notes) VALUES ($1, $2, $3, $4, $5, $6)', [
            ...
            if (status === 'completed') {
                ...
                for (const item of cycleCount.items) {
                    if (item.discrepancy !== 0) {
                        // Ajustar estoque
                        await client.query('UPDATE product_variations SET stock_quantity = stock_quantity + $1 WHERE id = $2', [item.discrepancy, item.product_variation_id]);
