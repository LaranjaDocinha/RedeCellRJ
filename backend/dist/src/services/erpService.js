...
export const erpService = {
    async exportSalesToERP(startDate, endDate) {
        ...
        const salesRes = await pool.query(`SELECT
         s.id AS sale_id,
         s.total_amount,
         s.sale_date,
       ...
       ORDER BY s.sale_date ASC`, [startDate, endDate]);
    ...
    async exportExpensesToERP(startDate, endDate) {
        ...
        const expensesRes = await pool.query(`SELECT
         id AS expense_id,
         amount,
         description,
         expense_date,
       ...
       ORDER BY expense_date ASC`, [startDate, endDate]);
