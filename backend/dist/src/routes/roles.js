...
// Update a role by ID
rolesRouter.put('/:id', validate(updateRoleSchema), async (req, res, next) => {
    try {
...
rolesRouter.post('/:roleId/permissions', validate(assignPermissionSchema), async (req, res, next) => {
    try {
...
rolesRouter.delete('/:roleId/permissions/:permissionId', async (req, res, next) => {
    try {
        const roleId = parseInt(req.params.roleId);
