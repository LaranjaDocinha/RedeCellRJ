...
// Get all customers
customersRouter.get('/', authMiddleware.authorize('read', 'Customer'), cacheMiddleware(), // Aplicar cacheMiddleware aqui
async (req, res, next) => {
    try {
...
// Search customers
customersRouter.get('/search', authMiddleware.authorize('read', 'Customer'), async (req, res, next) => {
    try {
...
customersRouter.get('/segments', authMiddleware.authorize('read', 'Customer'), async (req, res, next) => {
    try {
...
// Get customer by ID
customersRouter.get('/:id', authMiddleware.authorize('read', 'Customer'), async (req, res, next) => {
    try {
...
customersRouter.get('/:id/360view', authMiddleware.authorize('read', 'Customer'), async (req, res, next) => {
    try {
...
customersRouter.get('/:customerId/communications', authMiddleware.authorize('read', 'Communication'), // Or 'read', 'Customer'
async (req, res, next) => {
    try {
        const { customerId } = req.params;
        const communications = await customerCommunicationService.getCommunicationsForCustomer(parseInt(customerId));
...
customersRouter.post('/', authMiddleware.authorize('create', 'Customer'), validate(createCustomerSchema), async (req, res, next) => {
    try {
...
customersRouter.put('/:id', authMiddleware.authorize('update', 'Customer'), validate(updateCustomerSchema), async (req, res, next) => {
    try {
...
customersRouter.delete('/:id', authMiddleware.authorize('delete', 'Customer'), async (req, res, next) => {
    try {
...
customersRouter.post('/:id/loyalty/add', authMiddleware.authorize('update', 'Customer'), // Assuming update permission for loyalty
validate(loyaltyPointsSchema), async (req, res, next) => {
    try {
...
customersRouter.post('/:id/loyalty/subtract', authMiddleware.authorize('update', 'Customer'), // Assuming update permission for loyalty
validate(loyaltyPointsSchema), async (req, res, next) => {
    try {
        const { points } = req.body;
...
customersRouter.post('/:id/store-credit/add', authMiddleware.authorize('update', 'Customer'), validate(storeCreditSchema), async (req, res, next) => {
    try {
        const { amount, reason } = req.body;
        const updatedCustomer = await customerService.addStoreCredit(req.params.id, amount, reason, // Pass reason
...
customersRouter.post('/:id/store-credit/deduct', authMiddleware.authorize('update', 'Customer'), validate(storeCreditSchema), async (req, res, next) => {
    try {
        const { amount, reason } = req.body;
        const updatedCustomer = await customerService.deductStoreCredit(req.params.id, amount, reason, // Pass reason
        null // TODO: req.client was here, needs a transaction middleware
        ); // Pass client for transaction
        if (!updatedCustomer) {
            throw new AppError('Customer not found', 404);
...
customersRouter.get('/:id/store-credit/transactions', authMiddleware.authorize('read', 'Customer'), async (req, res, next) => {
    try {
...
customersRouter.post('/upload-document', authMiddleware.authorize('create', 'Customer'), // Permissão para criar cliente
