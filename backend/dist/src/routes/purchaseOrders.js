...
purchaseOrdersRouter.post('/', validate(createPurchaseOrderSchema), async (req, res, next) => {
    try {
...
purchaseOrdersRouter.put('/:id', validate(updatePurchaseOrderSchema), async (req, res, next) => {
    try {
        const updatedOrder = await purchaseOrderService.updatePurchaseOrder(parseInt(req.params.id), req.body);
        if (!updatedOrder) {
            throw new AppError('Purchase order not found', 404);
        }
