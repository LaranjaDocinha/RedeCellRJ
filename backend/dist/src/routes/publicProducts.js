...
const getProductsQuerySchema = z.object({
    ...
    categoryId: z.preprocess((val) => parseInt(val, 10), z.number().int().positive()).optional(),
    minPrice: z.preprocess((val) => parseFloat(val), z.number().positive()).optional(),
    maxPrice: z.preprocess((val) => parseFloat(val), z.number().positive()).optional(),
    sortBy: z.enum(['name', 'price', 'created_at']).default('name').optional(),
    sortDirection: z.enum(['ASC', 'DESC']).default('ASC').optional(),
    limit: z.preprocess((val) => parseInt(val, 10), z.number().int().positive()).default(10),
...
router.get('/', validateQuery(getProductsQuerySchema), async (req, res, next) => {
    try {
        const options = req.query; // Cast validado
