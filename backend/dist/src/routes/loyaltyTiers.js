...
loyaltyTiersRouter.get('/', async (req, res, next) => {
    try {
...
loyaltyTiersRouter.get('/:id', async (req, res, next) => {
    try {
...
loyaltyTiersRouter.post('/', validate(createLoyaltyTierSchema), async (req, res, next) => {
    try {
...
loyaltyTiersRouter.put('/:id', validate(updateLoyaltyTierSchema), async (req, res, next) => {
    try {
        const { name, min_points, description, benefits } = req.body;
        const updatedTier = await loyaltyTierService.updateTier(parseInt(req.params.id), name, min_points, description, benefits);
        if (!updatedTier) {
            throw new AppError('Loyalty tier not found', 404);
        }
...
loyaltyTiersRouter.delete('/:id', async (req, res, next) => {
    try {
