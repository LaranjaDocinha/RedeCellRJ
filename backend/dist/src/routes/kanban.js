...
router.get('/', authMiddleware.authenticate, authMiddleware.authorize('read', 'KanbanTask'), async (req, res, next) => {
    try {
...
router.put('/cards/move', authMiddleware.authenticate, authMiddleware.authorize('update', 'KanbanTask'), validate(moveCardSchema), async (req, res, next) => {
    try {
...
router.post('/cards', authMiddleware.authenticate, authMiddleware.authorize('create', 'KanbanTask'), validate(createCardSchema), async (req, res, next) => {
    try {
...
const updateCardSchema = z
    ...
    .partial();
router.put('/cards/:id', authMiddleware.authenticate, authMiddleware.authorize('update', 'KanbanTask'), validate(updateCardSchema), async (req, res, next) => {
    try {
...
router.delete('/cards/:id', authMiddleware.authenticate, authMiddleware.authorize('delete', 'KanbanTask'), async (req, res, next) => {
    try {
        const cardId = parseInt(req.params.id, 10);
