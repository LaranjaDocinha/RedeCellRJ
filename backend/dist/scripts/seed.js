...
const SUBJECTS = ['Audit', 'Branch', 'Category', 'Coupon', 'Customer', 'Dashboard', 'Discount', 'Inventory', 'KanbanTask', 'Loyalty', 'LoyaltyTier', 'Order', 'Payment', 'Permission', 'Product', 'ProductKit', 'PurchaseOrder', 'Report', 'Return', 'Review', 'Role', 'Sale', 'Search', 'Settings', 'Supplier', 'Tag', 'Upload', 'User', 'UserDashboard'];
const permissions = SUBJECTS.flatMap(subject => ALL_ACTIONS.map(action => ({ action, subject })));
const roles = {
    admin: {
        name: 'admin',
...
async function seedRbac(client) {
    ...
    for (const p of permissions) {
        const res = await client.query('INSERT INTO permissions (action, subject) VALUES ($1, $2) ON CONFLICT (action, subject) DO UPDATE SET subject = EXCLUDED.subject RETURNING id', [p.action, p.subject]);
    ...
    for (const roleKey in roles) {
        ...
        const res = await client.query('INSERT INTO roles (name) VALUES ($1) ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name RETURNING id', [role.name]);
...
// ========== User Seeding ==========
async function seedUsers(client, roleIds, numberOfUsers) {
    ...
    const adminUserRes = await client.query(`INSERT INTO users (name, email, password_hash) 
     VALUES ($1, $2, $3) 
     ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name, password_hash = EXCLUDED.password_hash
     RETURNING id`, ['Admin User', adminEmail, adminPasswordHash]);
    ...
    for (let i = 0; i < numberOfUsers; i++) {
        ...
        // Insert user
        const userRes = await client.query(`INSERT INTO users (name, email, password_hash) 
       VALUES ($1, $2, $3) 
       ON CONFLICT (email) DO NOTHING 
       RETURNING id`, [`${firstName} ${lastName}`, email, passwordHash]);
        if (userRes.rows[0]) {
...
export async function runAllSeeds() {
    ...
    try {
        ...
        for (const supplier of suppliers) {
            const res = await pool.query('INSERT INTO suppliers (name, contact_person, email) VALUES ($1, $2, $3) ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name RETURNING id', [supplier.name, supplier.contact_person, supplier.email]);
        ...
        for (const part of parts) {
            const partRes = await pool.query('INSERT INTO parts (name, sku, stock_quantity) VALUES ($1, $2, $3) ON CONFLICT (sku) DO UPDATE SET name = EXCLUDED.name, stock_quantity = EXCLUDED.stock_quantity RETURNING id', [part.name, part.sku, part.stock_quantity]);
            const partId = partRes.rows[0].id;
            await pool.query('INSERT INTO part_suppliers (part_id, supplier_id, cost) VALUES ($1, $2, $3) ON CONFLICT (part_id, supplier_id) DO UPDATE SET cost = EXCLUDED.cost', [partId, part.supplier_id, part.cost]);
        }
